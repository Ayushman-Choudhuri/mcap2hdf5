...
class HDF5Writer:
    ...
    def createDatasets(self, sampleTemplate):
        ...
        self.h5File.create_dataset(
            TIMESTAMP_DATASET_PATH, shape=(0,), maxshape=(None,), dtype=np.float64
        )
        self.h5File.create_dataset(
            CHUNK_IDS_DATASET_PATH, shape=(0,), maxshape=(None,), dtype=np.int32
        )
        ...
        self.h5File.create_dataset(
            LIDAR_OFFSETS_DATASET_PATH, shape=(0,), maxshape=(None,), dtype=np.int64
        )
        self.h5File.create_dataset(
            LIDAR_COUNTS_DATASET_PATH, shape=(0,), maxshape=(None,), dtype=np.int32
        )
    ...
    def resizeDatasets(self, newSize):
        ...
        self.h5File[CAMERA_IMAGES_DATASET_PATH].resize(
            (
                newSize,
    ...
    def finalize(self, cameraMetadata, staticTransforms):
        ...
        if cameraMetadata:
            self.h5File.attrs[CAMERA_K_MATRIX_ATTRIBUTE] = np.array(
                cameraMetadata.k, dtype=np.float32
            ).reshape(3, 3)
            self.h5File.attrs[CAMERA_D_MATRIX_ATTRIBUTE] = np.array(
                cameraMetadata.d, dtype=np.float32
            )
            self.h5File.attrs[CAMERA_R_MATRIX_ATTRIBUTE] = np.array(
                cameraMetadata.r, dtype=np.float32
            ).reshape(3, 3)
            self.h5File.attrs[CAMERA_P_MATRIX_ATTRIBUTE] = np.array(
                cameraMetadata.p, dtype=np.float32
            ).reshape(3, 4)
